...
import StatsCard from "@/Components/Home/ResultsStatsSection/StatsCard/StatsCard";
const AboutBanner = () => {
    console.log(aboutStatsData,'this about stats daata')
  return (
    <section
 ...
 <h1 className="text-4xl md:text-6xl lg:text-5xl xl:text-[80px] font-dmSans font-bold lg:font-extrabold text-[#060606]">
            ...
            We&lsquo;re more than a digital marketing agency. We&lsquo;re your partner in Bangladesh&lsquo;s 
fast-changing digital landscape. Delivering measurable growth 
through ethical, data-driven, and AI-powered strategies.
          </p>
        </div>
        {/* button */}
        <div className="flex flex-col md:flex-row gap-6 items-center">
            <Button>Explore Our Services <span className="ml-2 text-xl"><FaArrowRightLong/></span></Button>

            <button className=" py-3 px-6 border border-primary font-inter rounded-lg text-primary">View Case Studies</button>
        </div>
      ...
      <div className="max-w-250 mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-center justify-center pt-16">
         {
        aboutStatsData.map((stat,i)=> <div key={i} 
        className="py-5 px-15 bg-[#FFFFFF] border border-[#E6F2FF] shadow-sm rounded-[20px] hover:bg-[#f0f7ff] hover:border-primary transition-all duration-500 text-center ">
              <Image
                ...
                className="mx-auto "
              />
              <h2 className={`${stat?.titleColor} mt-6 mb-3 text-2xl md:text-3xl font-semibold lg:font-bold lg:text-[32px]`}>
                {stat.title}
              </h2>
